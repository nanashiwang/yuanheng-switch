...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("yuanhengswitch://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.yuanheng-switch/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/yuanheng-switch.log）
            {
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("yuanheng-switch".into()),
                            }),
                        ])
                ...
                log::info!("=== YuanHeng Switch v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 YuanHeng Switch 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes below the app identifier's data directory.
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/yuanheng-switch-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first yuanhengswitch:// URL
                        }
                    }
                }
            });
            ...
            let mut tray_builder = TrayIconBuilder::with_id(tray::TRAY_ID)
                .tooltip("YuanHeng Switch") // 鼠标悬停提示
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 yuanhengswitch://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        format!(
            "从旧版本迁移配置时发生错误：\n\n{error}\n\n\
            您的数据尚未丢失，旧配置文件仍然保留。\n\
            建议回退到旧版本 YuanHeng Switch 以保护数据。\n\n\
            点击「重试」重新尝试迁移\n\
        ...
        format!(
            ...
            Consider rolling back to an older YuanHeng Switch version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        format!(
            ...
            建议：\n\
            1) 先备份整个配置目录（包含 yuanheng-switch.db）\n\
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including yuanheng-switch.db)\n\
            2) If you see “database version is newer”, please upgrade YuanHeng Switch\n\

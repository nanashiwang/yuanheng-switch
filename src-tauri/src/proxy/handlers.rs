...
/// catalog with a top-level `models` field.  Return the yuanheng-switch–managed model
...
/// yuanheng-switch–owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
        ...
        active_catalog_path.as_ref().filter(|path| path.exists())
    {
        ...
        if active_catalog_path.is_none() {
            log::debug!(
                "[models] stale guard: catalog not served (model_catalog_json not set to yuanheng-switch catalog)"
            );
        }
        json!({"models": []})
    };
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 YuanHeng Switch 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a YuanHeng Switch limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "YuanHeng Switch local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("YuanHeng Switch local proxy failed"));

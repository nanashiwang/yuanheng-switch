...
///   且不一定等于用户目录，可能导致 `.yuanheng-switch/yuanheng-switch.db` 路径变化。
///
/// ## 测试隔离
///
/// 为了让 Windows CI/本地测试能稳定隔离真实用户数据，可通过 `YUANHENG_SWITCH_TEST_HOME`
/// 显式覆盖 home dir（仅用于测试/调试场景）。
pub fn get_home_dir() -> PathBuf {
    if let Ok(home) = std::env::var("YUANHENG_SWITCH_TEST_HOME") {
...
/// 获取应用配置目录路径 (~/.yuanheng-switch)
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    get_home_dir().join(".yuanheng-switch")
}

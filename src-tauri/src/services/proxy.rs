...
///
/// 原因：接管模式下 `*_MODEL` 必须由 YuanHeng Switch 写成稳定的 Claude 角色别名，
...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            let original_test_home = env::var("YUANHENG_SWITCH_TEST_HOME").ok();
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "yuanheng-switch-model-catalog.json must be created on provider switch"
        );
...
"#;
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        // Pre-takeover Live state: config.toml points at the yuanheng-switch generated
        ...
        assert!(
            ...
            "restored pointer must still reference the yuanheng-switch generated catalog file"
        );
    }
    ...
    async fn codex_restore_from_backup_projects_inline_model_catalog() {
        ...
        assert!(
            catalog_path.exists(),
            "restore must generate the yuanheng-switch catalog file on disk"
        );
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            ...
            "empty-auth restore must generate the yuanheng-switch catalog file"
        );
        assert!(

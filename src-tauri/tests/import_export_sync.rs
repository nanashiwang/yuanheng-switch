...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = yuanheng_switch_lib::get_codex_auth_path();
    let config_path = yuanheng_switch_lib::get_codex_config_path();
...
requires_openai_auth = true
"#;
    yuanheng_switch_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(yuanheng_switch_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = yuanheng_switch_lib::get_codex_config_path();
    ...
    yuanheng_switch_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = yuanheng_switch_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        yuanheng_switch_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        yuanheng_switch_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_single_server_to_codex_fails_closed_on_invalid_toml() {
    ...
    let err = yuanheng_switch_lib::sync_single_server_to_codex(
        &config,
        "srv",
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        yuanheng_switch_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !yuanheng_switch_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !yuanheng_switch_lib::get_codex_config_path().exists(),
...
"#;
    ...
    yuanheng_switch_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        yuanheng_switch_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = yuanheng_switch_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        yuanheng_switch_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        yuanheng_switch_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = yuanheng_switch_lib::import_from_codex(&mut config).expect("import codex");
...
"#,
    ...
    config.mcp.servers.as_mut().unwrap().insert(
        ...
        yuanheng_switch_lib::McpServer {
            ...
            apps: yuanheng_switch_lib::McpApps {
                claude: false,
                codex: false, // 初始未启用
                gemini: false,
                grokbuild: false,
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    yuanheng_switch_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = yuanheng_switch_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = yuanheng_switch_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".yuanheng-switch").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".yuanheng-switch");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.yuanheng-switch/settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("yuanheng-switch-test-invalid<>dir")
    } else {
...
fn import_sql_rejects_non_cc_switch_backup() {
    ...
    let import_path = home.join("not-yuanheng-switch.sql");
    fs::write(&import_path, "CREATE TABLE x (id INTEGER);").expect("write import sql");

    let err = state
        .db
        .import_sql(&import_path)
        .expect_err("non-yuanheng-switch sql should be rejected");
...
fn import_sql_accepts_cc_switch_exported_backup() {
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("yuanheng-switch-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");

...
describe("OpenCodeFormFields", () => {
  it("surfaces existing provider headers", () => {
    ...
    expect(screen.getByDisplayValue("YuanHeng Switch")).toBeInTheDocument();
  });

  it("updates provider headers", () => {
    const onHeadersChange = vi.fn();
    renderOpenCodeForm({
      headers: { "X-Title": "YuanHeng Switch" },
      onHeadersChange,
    });

    fireEvent.change(screen.getByDisplayValue("YuanHeng Switch"), {

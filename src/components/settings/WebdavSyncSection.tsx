...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "yuanheng-switch-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "yuanheng-switch-sync",
  ...
  const [s3RemoteRoot, setS3RemoteRoot] = useState(
    s3Config?.remoteRoot ?? "yuanheng-switch-sync",
  );
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "yuanheng-switch-sync";
  ...
  useEffect(() => {
    ...
    setS3RemoteRoot(s3Config.remoteRoot ?? "yuanheng-switch-sync");
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "yuanheng-switch-sync",
  ...
  const buildS3Settings = useCallback((): S3SyncSettings => {
    return {
      ...
      remoteRoot: s3RemoteRoot.trim() || "yuanheng-switch-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "yuanheng-switch-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
  const currentS3RemotePath = `${s3Bucket.trim() || "bucket"}/${s3RemoteRoot.trim() || "yuanheng-switch-sync"}/v2/db-v6/${s3Profile.trim() || "default"}`;
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <Input
                ...
                placeholder="yuanheng-switch-sync"
                className="text-xs flex-1"
                disabled={isLoading}
              />
            </div>

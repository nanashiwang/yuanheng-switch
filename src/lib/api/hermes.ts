...
/**
 * Hermes Agent configuration API (YuanHeng Switch side).
 *
 * YuanHeng Switch intentionally keeps its Hermes surface minimal — deep configuration
 * (model, agent behavior, env vars, skills, cron, logs, analytics) lives in
 * the Hermes Web UI at http://127.0.0.1:9119. YuanHeng Switch only reads the `model`
